    ...
    public class MatchHandler :AbsOnceHandler, HandlerInterface
    {
        ...
        private void leave(UserToken token)
        {
            ...
            if(roomMap.ContainsKey(roomId))
            {
                ...
                //根据用户所在的队伍进行移除
                if (room.teamOne.Contains(userid)) room.teamOne.Remove(userid);
                else if (room.teamTwo.Contains(userid)) room.teamTwo.Remove(userid);
                //移除用户与房间之间的映射关系
                userRoom.TryRemove(userid, out roomId);
                //如果当前用户为此房间的最后一人 则移除房间 并放进缓存队列
